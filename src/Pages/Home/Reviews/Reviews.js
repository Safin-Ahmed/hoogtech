...
const Reviews = () => {
  ...
  useEffect(() => {
    fetch("https://hoogtech-server.up.railway.app/reviews")
      .then((res) => res.json())
      ...
      <div className="container mx-auto">
        <div className="reviews-container flex flex-wrap">
          <div className="reviews-title w-full md:w-1/4">
            <h2 className="hidden md:block leading-loose text-center md:text-left text-5xl font-bold italic text-white">
              What
              <br />
              People Are
              <br />
          ...
          <div className="reviews-cards px-5 md:px-0 grid gap-4 grid-cols-1 md:grid-cols-3 w-full md:w-3/4">
            {reviews.map((review) => (
              <Review review={review} id={review._id}></Review>
            ))}
          </div>
        </div>
      </div>
