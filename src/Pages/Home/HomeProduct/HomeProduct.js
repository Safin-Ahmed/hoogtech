...
const HomeProduct = ({ product }) => {
    const { imgUrl, productName, desc, price, id } = product;
    return (
        <div className="rounded-xl product-card p-10 shadow-xl text-center  transition duration-150 ease-in-out transform hover:-translate-y-8">
            ...
            <button className="transition duration-150 ease-in-out rounded-lg hover:bg-black primary-bg text-xl text-white px-8 py-3 mt-4"><Link to={`/purchase/${id}`}>Purchase Now</Link></button>
        </div>
    );
};
