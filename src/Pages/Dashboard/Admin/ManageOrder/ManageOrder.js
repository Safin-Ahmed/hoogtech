...
const ManageOrder = ({ order, setManageOrders, token }) => {
  ...
  const handleShipped = () => {
    const confirmation = window.confirm(
      "Are you sure you want to mark this order shipped?"
    );
    const url = `https://hoogtech-server.up.railway.app/orders?id=${_id}&&status=shipped`;
    if (confirmation) {
      fetch(url, {
        method: "PUT",
        headers: {
        ...
        .then((data) => {
          if (data.modifiedCount) {
            ...
            fetch(`https://hoogtech-server.up.railway.app/allOrders`, {
              headers: {
